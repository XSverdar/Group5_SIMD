...
- **For SIMD YMM Implementation:** Initially, the consideration was to use horizontal addition to solve the project. However, upon studying the XMM implementation, it became clear that there was a simpler way to code the project. Drawing from the XMM implementation for the code, a problem was encountered regarding how much should be added to the index of YMM. There was some experimentation with numbers to guess the index until a groupmate recalled that the index is the same as XMM; however, the 'add rcx' is the only element to be changed - the AHA moments. Comparing this algorithm to my previous one, this one is more straightforward, easier to code, and less complex to trace.
